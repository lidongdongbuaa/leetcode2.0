...
C.Binary search 
    Method:
        1. corner case
        2. use mid pivot to divide nums as left and right
            check the mid val == target
            if left val == mid val, move left boundary of left part to get sorted part
                1,2,1 -> 2,1
            do the same thing as above problem
    Time complexity: worst O(N)
    Space: O(1)

易错点：若有相等的，则改变l，最终去得到sorted array elif nums[l] == nums[mid]: l += 1
