...
数学证明：
链表长L=F+a+b,F是acyclic部分，a加b是cyclic部分，设a+b=c，a与b的交点是快慢点交点
由于快慢交叉时，时间相等，速度差一倍，快点跑完F+n(a+b)+a, 慢点跑完了F+a
故(F+a)/1=(F+n(a+b)+a)/2->2(F+a)=F+nc+a->F+a=nc，F=nc-a
故慢点从起始点开跑，跑完F部分时，t=F/1
此时快点以慢点的速度从交叉处F+a开跑，跑了t*1=F=nc-a路程，
故此时快点的位置为F+a+(F mod c)=F+a+(nc-a)mod c=F+(a+nc-a)mod c=F
故此时快慢点将在交叉处遇到
