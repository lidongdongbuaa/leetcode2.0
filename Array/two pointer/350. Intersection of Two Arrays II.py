...
方法及方法分析：dict save and count；Sorted and two pointer
time complexity order: dict save and count O(m + n) < Sorted and two pointer O(nlogn + mlogm)
space complexity order:Sorted and two pointer O(1) <  dict save and count O(min(m,n))
如何考
'''
'''
...
C. !!Sorted and two pointer
    Method:
        1. corner case
        2. sorted nums1 and 2
        3. use two pointer to compare the nums, and add the common to result
