...
2.理解：删除倒数第n个节点
3.类型：删除题
4.方法及方法分析：next跳跃法；list重构法；one-pass(双指针-数组)法
time complexity order: next跳跃法 O(N) =list重构法 O(N) = one-pass(双指针-数组)法 O(N)
space complexity order: next跳跃法 O(1) = one-pass(双指针-数组)法 O(1) < list重构法 O(N) 
'''
